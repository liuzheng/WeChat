...
.controller('appController', [
    ...
    function($rootScope,$scope, $timeout,$log,$state,$window,
             ...
             confFactory,contextMenuFactory, notificationFactory, utilFactory,reportService,surviveCheckService,subscribeMsgService,stateManageService) {
        ...
        $scope.$on('newLoginPage',function (e,data) {
            ...
            //if(!data.Passticket){ alert('newLoginPage error'); return;}
        ...
        function pageInit () {
            ...
            appFactory.init().then(function (data) {
                ...
                if(data.BaseResponse && data.BaseResponse.Ret != "0"){
                    ...
                    if (!loginFactory.timeoutDetect(data.BaseResponse.Ret)) {
                        ngDialog.openConfirm({
                            ...
                            controller:['$scope',function(scope){
                                scope.title = MM.context("02d9819");
                                scope.content = MM.context("0d2fc2c");
                ...
                $timeout(function () {
                    ...
                    contactFactory.initContact().then(function (data) {
        ...
        $scope.isCloseGiveMeFive = !$scope.isWindows || utilFactory.getLocalStorage().getItem('close_ad');

        if (!$scope.isCloseGiveMeFive){
            reportService.report(reportService.ReportType.WinAdPV, {
                count: 1
            });
        }

        // 骚扰，直接叉掉！
        $scope.closeGiveMeFive = function () {
            $scope.isCloseGiveMeFive = true;
            reportService.report(reportService.ReportType.click2CloseAd, {
                count: 1
            });
            utilFactory.getLocalStorage().setItem('close_ad', '1');
        };
        // 嗯，还是好奇点一下！
...
angular.module('Controllers')
.controller('loginController', ['$scope', 'loginFactory','utilFactory','reportService', function($scope, loginFactory, utilFactory,reportService) {
    ...
    if(window.MMCgi.isLogin) return;
            ...
            loginFactory.getUUID().then(function (uuid) {
                ...
                function checkLoginHandler (data) {
                    ...
                    switch(data.code){
                        ...
                        case 201:
                            ...
                            loginFactory.checkLogin(uuid).then(checkLoginHandler);
                            break;
                        case 408:
                            loginFactory.checkLogin(uuid).then(checkLoginHandler);
                            break;
                ...
                if(!window.MMCgi.isLogin){
                    loginFactory.checkLogin(uuid,1).then(checkLoginHandler);
                }
            ...
            function($scope, $timeout,$state,$log,$document,$compile,
                ...
                $scope.showProfile = function (e,userName,isAdd) {
                    ...
                    mmpop.open({
                        ...
                        controller:["$scope",function (scope) {
                            scope.contact = contact;
                            scope.MMDefaultRemark = MM.context("8d521cc");
                            scope.addUserContent = MM.context("5a97440") + emojiFactory.formatHTMLToSend(accountFactory.getUserInfo().NickName);
                            ...
                            scope.editRemarkName = function(){
                                if(scope.MMDefaultRemark == MM.context("8d521cc")){
                ...
                $scope.$watchCollection('chatContent',function(list){
                    if(list.length > 0 &&list[list.length-1].FromUserName === accountFactory.getUserName()){
                        ...
                        shouldScroll = chatBd.scrollTop + chatBd.clientHeight + 10 >= chatBd.scrollHeight;
                        shouldCalc = false;
                    }
...
.controller('chatSenderController', [
    ...
    'emojiFactory',
        'reportService',
    function($rootScope, $scope,$http,ngDialog,confFactory,accountFactory,contactFactory,chatFactory,screenShotFactory, utilFactory,mmpop,stateManageService,emojiFactory,reportService) {
        ...
        function _insertToEditArea(ctn, isEditing){
            ...
            if (window.getSelection) { // IE9 and non-IE
                ...
                range = isEditing ? document.selection.createRange() : editAreaRange;
                range.pasteHTML(ctn);
                range.select();
            }
        }
        ...
        $scope.screenShot = function(){
            ...
            if (screenShotFactory.isSupport()) {
                ...
                if(navigator.platform == "Win64" && utilFactory.browser.msie) alert(MM.context("82cf63d"));
                else if(confirm(MM.context("112a5c0"))) screenShotFactory.install();
            }
        };
        ...
        var MAX_IMG_UPLOAD_SIZE = 10;//10M
        var MAX_FILE_UPLOAD_SIZE = 20;//20M
            ...
            chooseFileCbs = { // 选择文件 背景下的回调
                onQueued: function(){
                    ...
                    this.MMSendMsg = chatFactory.createMessage({ // 创建消息
                        MsgType: isImg(this.ext) ? confFactory.MSGTYPE_IMAGE : confFactory.MSGTYPE_APP,
                    ...
                    if(isImg(this.ext)){
                        ...
                        chatFactory.appendMessage(this.MMSendMsg);
                    }
                },
...
/*                    console.error(me.name + " uploadError: ", reason);*/
            ...
            pasteImgCbs = {
                ...
                onError: function(reason){
                    ...
                    alert(MM.context("c5795a7") + reason);
                }
            };
        /**
        ...
        try{
            var uploader = WebUploader.create({
                ...
                .on("beforeFileQueued", function(file){ // 添加到队列前触发
                    if(isImg(file.ext)){
                        if(file.size > MAX_IMG_UPLOAD_SIZE * 1024 * 1024){
                            alert(MM.context("8c88ff6") + MAX_IMG_UPLOAD_SIZE + "M");
                            return false;
                        }
                    }else{
                        if(file.size > MAX_FILE_UPLOAD_SIZE * 1024 * 1024){
                            alert(MM.context("0c9c48a") + MAX_FILE_UPLOAD_SIZE + "M");
                            return false;
                        }
                    }

                    if(/untitled\d+.png/i.test(file.name)) fileExtend(file, pasteImgCbs); // TODO 暂时只能以这么龊的方式区分是file是来自 剪切板 还是 文件选择器
                    else fileExtend(file, chooseFileCbs); // 文件必须有对应状态的回调方法，这里是当来到这里没有时，当作来自 文件选择器 处理
                })
                ...
                .on("uploadBeforeSend", function(obj, data, headers){ // 在文件发送前触发
                    ...
                    data.mediatype = isImg(file.ext) ? "pic" : "doc";
                    data.uploadmediarequest = JSON.stringify(angular.extend(accountFactory.getBaseRequest(),{
...
.controller('createChatroomController', [
    ...
    'stateManageService',
        'accountFactory',
    function($rootScope, $scope, $timeout,$state, $log, $document, chatFactory,contactFactory, appFactory, chatroomFactory,confFactory, mmpop, ngDialog, utilFactory,stateManageService,accountFactory) {
        ...
        var searchTimer,
            searchList,
            searFilterContacts,
            filterContacts = chatroomFactory.getFilterContacts();
        function _pickContact(){
        ...
        $scope.add = function () {
            ...
            chatroomFactory.addMember(currentContact.UserName,addMemberUserNames.join(','),function (data) {
                console.log('add member to chatroom ok',data);
            ...
            chatroomFactory.create(memberList).then(function (data) {
                if(data.BaseResponse && data.BaseResponse.Ret == 0){
                    ...
                    ngDialog.openConfirm({
                        ...
                        controller:['$scope',function(scope){
                            scope.title = MM.context("02d9819");
                            scope.content = MM.context("0d42740");
                            scope.callback = function () {
                                scope.closeThisDialog();
                            };
                        }]
        ...
        .controller('contextMenuController', [
           ...
           /* function showNavContactContextMenu(userName) {
            ...
            function showMessageContextMenu(actualSenderUserName, msgType,subType, msgId, event) {
                ...
                if(/*msg.MsgType == confFactory.MSGTYPE_IMAGE
                    ||*/ msg.MsgType == confFactory.MSGTYPE_TEXT
                    /*|| msg.MsgType == confFactory.MSGTYPE_EMOTICON*/
                    || (msg.MsgType == confFactory.MSGTYPE_APP && msg.AppMsgType == confFactory.APPMSGTYPE_ATTACH)){
                    $scope.contextMenuList.push({
                        content:MM.context("21e106f"),
                        callback:function () {
                            ngDialog.open({
...
.controller('systemMenuController', [
    ...
    'utilFactory',
    'oplogFactory',
    function($rootScope,$scope, $timeout, ngDialog, loginFactory,confFactory, accountFactory,utilFactory,oplogFactory) {
...
*/
                ...
                function sendMsg(msg,to){
                    ...
                    transpondMsg.Content= transpondMsg.MMActualContent;
    ...
    var service = {
        ...
        syncCheck: function () {
            ...
            var url = confFactory.API_synccheck+'?'+[
                    ...
                    'uin='+accountFactory.getUin(),
                    'deviceid='+accountFactory.getDeviceID(),
                    'synckey='+encodeURIComponent(accountFactory.getFormateSyncKey())
            ].join('&');
...
.factory('chatFactory', [
    ...
    var service = {
        ...
        postAppMessage: function(msg){
            var data = {
                    Type: confFactory.APPMSGTYPE_ATTACH,//兼容后台而带上Type
                    Content:
            ...
            this._postMessage(confFactory.API_webwxsendappmsg + "?fun=async&f=json", data, msg);
        },
        // 发送表情消息
        ...
        messageProcess: function (message) {
            ...
            switch(message.MsgType){
                ...
                case confFactory.MSGTYPE_SYS://系统消息
                    ...
                    break;
                default:
                    message.MMDigest = MM.context("938b111");
            }
        ...
        _imageMsgProcess: function (message) {
            ...
            message.MMDigest += MM.context("a5627e8");
        },
        /**
         * 语音消息处理
         * @param message 消息体
        ...
        _voiceMsgProcess: function (message) {
            ...
            message.MMDigest += MM.context("b28dac0");
        ...
        _videoMsgProcess: function (message) {
            ...
            message.MMDigest += MM.context("4078104");
        },
        /**
         * 小视频消息处理
         * @param message 消息体
        ...
        _mircovideoMsgProcess: function (message) {
            ...
            message.MMDigest += MM.context("1f94b1b");
        },
        /**
         * news消息处理
         * @param message 消息体
        ...
        _emojiMsgProcess : function(message) {
            ...
            if(message.HasProductId){
                if(message.MMIsSend){
                    message.MMActualContent = ''+MM.context("80f56fb");
                }else{
                    message.MMActualContent = MM.context("2242ac7") + '';
                }
                me._textMsgProcess(message);
            }else{
                ...
                message.MMDigest += MM.context("e230fc1");
            }
        ...
        _voipMsgProcess: function (message) {
            this._appAsTextMsgProcess(message, MM.context("fdaa3a3"));
        },
        /**
         * 地理位置消息处理
         * @param message 消息体
        ...
        _locationMsgProcess: function (message) {
            ...
            //this._appAsTextMsgProcess(message, MM.context("2cc0cfc"));
        },
        /**
         * app消息处理
         * @param message 消息体
        ...
        _appTextMsgProcess: function (message) {
            ...
            content = utilFactory.encodeEmoji(content);
            content = utilFactory.xml2json(content);
            this._appAsTextMsgProcess(message, utilFactory.decodeEmoji(content.appmsg.title));
        },
        /**
         * app语音消息
         * @param  {[type]} message
        ...
        _appAudioMsgProcess: function (message) {
            var me = this,
                digest = MM.context("0e23719") + message.FileName;
        ...
        _appVideoMsgProcess: function (message) {
            var me = this,
                digest = MM.context("4078104") + message.FileName;
        ...
        _appOpenMsgProcess: function (message) {
            var me = this,
                digest = MM.context("4f20785");

            me._appUrlMsgProcess(message,digest);
            message.MMAlert = MM.context("c4e04ee");
        },
        /**
         * app链接消息
         * @param  {[type]} message
        ...
        _appUrlMsgProcess: function (message,digest) {
            ...
            digest = digest || MM.context("e5b228c") + message.FileName;
        ...
        _appReaderMsgProcess: function(message,readerObj){
            ...
            angular.forEach(message.MMCategory, function (item) {
                item.pub_time = jQuery.format.date(item.pub_time*1000,'MM-dd');
        ...
        _appAttachMsgProcess: function (message) {
            ...
            //console.log(content);
            message.MMDigest += MM.context("6daeae3");
        ...
        _appTransfersMsgProcess: function (message) {
            this._appAsTextMsgProcess(message, MM.context("0cdad09"));
        },
        /**
         * app卡券消息
         * @param  {[type]} message
        ...
        _appCardTicketMsgProcess: function (message) {
            this._appAsTextMsgProcess(message, MM.context("c534fc3"));
        },
        /**
         * app共享实时位置消息处理
         * @param message 消息体
        ...
        _appRealtimeShareLocationMsgProcess: function (message) {
            ...
            if(message.FromUserName == accountFactory.getUserName()){
                ...
                content = "[" + (displayName ? displayName : MM.context("a41d576")) + MM.context("a1f1299") + "]";
            }
        ...
        _appScanGoodMsgProcess: function (message) {
            this._appAsTextMsgProcess(message, MM.context("95afe20"));
        },
        /**
         * app商品消息处理
         * @param message 消息体
        ...
        _appGoodMsgProcess: function (message) {
            this._appAsTextMsgProcess(message, MM.context("355765a"));
        },
        /**
         * app表情分享消息处理
         * @param message 消息体
        ...
        _appEmotionMsgProcess: function (message) {
            this._appAsTextMsgProcess(message, MM.context("9d7f4bb"));
        },
        _appRedEnvelopesMsgProcess: function (message) {
            message.MMDigest += MM.context("e24e75c");
            //this._appAsTextMsgProcess(message, MM.context("c5e6ab0"));
        },
        /**
         * app未知消息，用于调试
         * @param  {[type]} message
        ...
        _recalledMsgProcess: function (message) {
            var me = this,
                ...
                msgIndex,
                actualSender,
                content = MM.context("ded861c"),
                userMessages = me.getChatMessage(message.MMPeerUserName);

            str = utilFactory.encodeEmoji(str);
            recallMsg = utilFactory.xml2json(str).revokemsg;
            ...
            if(msgIndex > -1){
                ...
                if(!oldMsg.MMIsSend){
                    ...
                    actualSender = MM.context("df1fd91");
                }
        ...
        _recommendMsgProcess : function(message) {
            ...
            if (message.MsgType == confFactory.MSGTYPE_VERIFYMSG) {
                message.Content = message.Contact.NickName||message.Contact.UserName + MM.context("ebeaf99");
            } else {
        ...
        _verifyMsgProcess : function(message) {
            message.MMDigest = message.RecommendInfo.NickName + MM.context("ebeaf99");
        ...
        _shareCardProcess : function (message) {
            if (message.MMActualSender == accountFactory.getUserName()) {
                message.MMDigest += MM.context("9a2223f") + message.RecommendInfo.NickName;
            } else {
                message.MMDigest += MM.context("dd14577") + message.RecommendInfo.NickName;
            }
            for(var attr in message.RecommendInfo){
                if(!message.RecommendInfo[attr]){
                    delete message.RecommendInfo[attr];
        ...
        _notify: function(message){
            ...
            function notify(){
                ...
                var content = message.MMDigest.replace(/(<img.*?\/>)|<span class="emoji.*?<\/span>/g, MM.context("809bb9d"));
...
function( $q, confFactory, utilFactory) {
    ...
    var service = {
        ...
        getFormateSyncKey: function() {
            var synckeyList = this.getSyncKey().List,
            synckeyFormat = [];
            for (var i = 0,
            len = synckeyList.length; i < len; i++) {
                synckeyFormat.push(synckeyList[i].Key + "_" + synckeyList[i].Val);
            }
            return synckeyFormat.join('|');
        },
...
.factory('confFactory', ['$q', function( $q) {
    ...
    if(host.indexOf("wx2.qq.com") > -1){
        ...
        pushHost = "webpush2.weixin.qq.com";
    }else if(host.indexOf("qq.com") > -1){
        baseHost = "weixin.qq.com";
        fileHost = "file.wx.qq.com";
        pushHost = "webpush.weixin.qq.com";

    }else if(host.indexOf("web1.wechat.com") > -1){
        baseHost = "wechat.com";
        fileHost = "file1.wechat.com";
        pushHost = "webpush1.wechat.com";
    }else if(host.indexOf("web2.wechat.com") > -1){
        baseHost = "wechat.com";
        fileHost = "file2.wechat.com";
        pushHost = "webpush2.wechat.com";
    }else if(host.indexOf("wechat.com") > -1){
        baseHost = "wechat.com";
        fileHost = "file.wechat.com";
        pushHost = "webpush.wechat.com";

    }else if(host.indexOf("web1.wechatapp.com") > -1){
        baseHost = "wechatapp.com";
        fileHost = "file1.wechatapp.com";
        pushHost = "webpush1.wechatapp.com";
    }else{
        baseHost = "wechatapp.com";
        fileHost = "file.wechatapp.com";
        pushHost = "webpush.wechatapp.com";
    }
    ...
    var service = {
        ...
        EMOTICON_REG: 'img\\sclass="(qq)?emoji (qq)?emoji([\\da-f]*?)"\\stext=".*?"\\ssrc=".*?"\\s?', // img class="emoji emoji11" text="[..]" src=".."


        RES_PATH: "/zh_CN/htmledition/v2/",
        // 需要用到RES_PATH的静态变量请参考 写在接近页面底部的 RES_IMG_DEFAULT

        API_jsLogin: "https://login." + baseHost + "/jslogin?appid=wx782c26e4c19acffb&redirect_uri="
                    + encodeURIComponent(location.protocol + "//" + location.host + "/cgi-bin/mmwebwx-bin/webwxnewloginpage") + "&fun=new&lang=" + lang,
        API_login: 'https://login.' + baseHost + '/cgi-bin/mmwebwx-bin/login',
...
.factory('contactFactory', [
    ...
    var _contactOperates = {
        ...
        isRoomOwner : function() {
            return this.isRoomContact() && this.OwnerUin == accountFactory.getUin();
        },
        /**
         * 是否为公众号
         */
        ...
        isInChatroom: function () {
            ...
            angular.forEach(this.MemberList,function (item) {
                if(item.Uin == accountFactory.getUserInfo().Uin){
        ...
        isReadOnlyContact: function () {
            return _readOnlyContacts.indexOf(this.Uin) > -1;
        },
        /**
         * 获取联系人`DisplayName`
        ...
        getMemberDisplayName: function (chatroomUserName) {
            ...
            if(chatroomId && _chatRoomMemberDisplayNames[chatroomId]){
                return _chatRoomMemberDisplayNames[chatroomId][this.UserName];
            }else{
                return '';
            }
        },
    ...
    var service = {
         ...
         * @param contact
         */
        addChatroomMemberDisplayName: function (contact, chatroomId) {
            if(!contact.DisplayName) return;

            if(chatroomId){
                if(!_chatRoomMemberDisplayNames[chatroomId]){
                    _chatRoomMemberDisplayNames[chatroomId] = {};
                }
                _chatRoomMemberDisplayNames[chatroomId][contact.UserName] = contact.DisplayName;
            }
        },
        getChatroomIdByUserName: function (chatroomUserName) {
            var chatroomContact = _contacts[chatroomUserName] || {};
            return chatroomContact.Uin;
        },
            ...
            function addContactsHandler (data) {
                ...
                angular.forEach(data.ContactList,function (contact) {
                    ...
                    if(utilFactory.isRoomContact(contact.UserName) && contact.MemberList && contact.MemberList.length){
                        angular.forEach(contact.MemberList,function (item) {
                            ...
                            if(!c || !c.isContact()){
                                ...
                                item.HeadImgUrl = utilFactory.getContactHeadImgUrl({
                                    ChatroomId:contact.Uin,
                                    UserName:item.UserName,
                                    Skey:accountFactory.getSkey()
                                });
                                me.addContact(item);
                            }
                            me.addChatroomMemberDisplayName(item,contact.Uin);
                        ...
                        me.addChatroomMemberDisplayName(contact,contact.ChatRoomId);
                    }
                });
        ...
        /**
         * batchget群成员
         * 因为batchget chatroom后端返回的群成员不包含地区信息，所以要对群非好友再batchget一次，fuck
         * @param userName
         */
                ...
                angular.forEach(chatroomContact.MemberList,function(item){
                    ...
                    if(memberContact && !memberContact.isContact() && !memberContact.MMFromBatchget){
                        me.addBatchgetContact({
                            ...
                            ChatRoomId:chatroomContact.Uin
                        });
                    }
                });
            }
...
.factory('utilFactory', [ '$q','$rootScope','confFactory',function( $q ,$rootScope, confFactory) {
    ...
    var service = {
        ...
        getContactHeadImgUrl: function (options) {
            return (this.isRoomContact(options.UserName) ? confFactory.API_webwxgetheadimg : confFactory.API_webwxgeticon) + '?seq=0&username=' + options.UserName + '&skey=' + options.Skey + (options.MsgId ? ('&msgid=' + options.MsgId) : '') + (options.ChatroomId ? ('&chatroomid=' + options.ChatroomId) : '');
        },
                ...
                var MsgRemind = {
                    ...
                    _toggle: function () {
                        ...
                        if (document.title == this.defaultTitle && this.unreadMsgNum > 0) {
                            document.title = MM.context("cfbf6f4") + '(' + this.unreadMsgNum + ')';
                        } else {
...
/*__inline("directives/videojsDirective.js");*/
(function(undefined){
    'use strict';
...
angular.module('Filters')
    ...
    .filter("emojiHideFilter", function(){
        return function (text, length, end){
            ...
            return text.replace(/<span class=.emoji.*?<\/span>/g, MM.context("809bb9d"));
        }
    })
    ...
    .filter("timeFormat",["utilFactory", function(utilFactory){
        var map = [MM.context("562d747"),MM.context("1603b06"),MM.context("b5a6a07"),MM.context("e60725e"),MM.context("170fc8e"),MM.context("eb79cea"),MM.context("2457513")];
